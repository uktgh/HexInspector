...
class HexView(ttk.Frame):
    def __init__(self, parent, buffer: MemoryBuffer, cache: HexCache, bytes_per_row: tk.IntVar, show_offset: tk.BooleanVar, show_ascii: tk.BooleanVar):
    ...
    def update_view(self):
        ...
        for i in range(0, len(hex_data), bytes_per_row * 2):
            ...
            line = f"{i//2:08X}  {hex_chunk:<{bytes_per_row*3}}  {ascii_chunk}\n" if self.show_offset.get() else f"{hex_chunk:<{bytes_per_row*3}}  {ascii_chunk}\n"
            
            if not self.show_ascii.get():
                line = line.split("  ")[0] + "\n"
